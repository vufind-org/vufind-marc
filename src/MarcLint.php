 ...
 *
 * Current MARC::Lint version used as basis for this module: 1.53
 *
 * PHP version 7
 *
...
class MarcLint
{
    ...
    {
        $this->parseRules();
        $this->data = new Lint\CodeData();
    }

    /**
     * Check the provided MARC record and return an array of warning messages.
     *
     * @param MarcReader $marc Record to check
     *
     * @return array
     */
    public function checkRecord(MarcReader $marc): array
    {
        // Reset warnings:
        $this->warnings = [];
     ...
     * @return void
     */
    protected function checkDuplicate1xx(MarcReader $marc): void
    {
        $count = 0;
        foreach (['100', '110', '111', '130'] as $field) {
            $count += count($marc->getFields($field));
        }
        if ($count > 1) {
            $this->warn(
     ...
     * @return void
     */
    protected function checkMissing245(MarcReader $marc): void
    {
        if (!$marc->getField('245')) {
     ...
     * @return void
     */
    protected function standardFieldChecks(MarcReader $marc): void
    {
        $fieldsSeen = [];
        foreach ($marc->getAllFields() as $current) {
            $tagNo = $current['tag'];
     ...
     */
    protected function check020(array $field, MarcReader $marc): void
    {
        foreach ($field['subfields'] as $current) {
            $data = $current['data'];
            // remove any hyphens
     ...
     * @SuppressWarnings(PHPMD.UnusedFormalParameter)
     */
    protected function check041(array $field, MarcReader $marc): void
    {
     ...
     * @SuppressWarnings(PHPMD.UnusedFormalParameter)
     */
    protected function check043(array $field, MarcReader $marc): void
    {
        foreach ($field['subfields'] as $sub) {
            if ('a' !== $sub['code']) {
                continue;
    ...
    protected function check245(array $field, MarcReader $marc): void
    {
        ...
        foreach ($tmp as $current) {
            $subfields[] = $current;
            $hasSubfields[$current['code']] = true;
        }
     ...
     * @return void
     */
    protected function checkArticle(array $field, MarcReader $marc): void
    {
        ...
        if ($tagNo == '880' && ($sub6 = $marc->getSubfield($field, '6'))) {
            $tagNo = substr($sub6, 0, 3);
        }
        ...
        $first_or_second = '';
        if (in_array($tagNo, [130, 630, 730])) {
            $ind = $field['i1'];
            $first_or_second = '1st';
        } elseif (in_array($tagNo, [240, 245, 440, 830])) {
            $ind = $field['i2'];
            $first_or_second = '2nd';
        } else {
            $this->warn(
        ...
        $title = $marc->getSubfield($field, 'a');
    ...
    protected function processRuleGroup($rules)
    {
        ...
        [$tag, $repeatable, $description] = explode(' ', $rules[0]);
        $this->rules[$tag] = [
